...
setDepthTest:function(a){var b=cc._renderContext;a?(b.clearDepth(1),b.enable(b.DEPTH_TEST),b.depthFunc(b.LEQUAL)):b.disable(b.DEPTH_TEST)},pushRenderCommand:function(a){if(a.rendering||a.uploadData)if(this._isCacheToBufferOn){var b=this._cacheToBufferCmds[this._currentID];-1===b.indexOf(a)&&b.push(a)}else-1===this._renderCmds.indexOf(a)&&this._renderCmds.push(a)},_increaseBatchingSize:function(a,b,c){b=b||u.QUAD;switch(b){case u.QUAD:for(b=0;b<a;b+=4)c=f+b,p[g++]=c+0,p[g++]=c+1,p[g++]=c+2,p[g++]=
c+1,p[g++]=c+2,p[g++]=c+3;break;case u.TRIANGLE:s=!1;for(b=0;b<a;b+=3)c=f+b,p[g++]=c+0,p[g++]=c+1,p[g++]=c+2;break;case u.CUSTOM:s=!1;var d=c.length;for(b=0;b<d;b++)p[g++]=f+c[b];break;default:return}f+=a},_updateBatchedInfo:function(b,c,d){b&&(a.texture=b);c&&(a.blendSrc=c.src,a.blendDst=c.dst);d&&(a.shader=d)},_breakBatch:function(){b=!0},_uploadBufferData:function(c){f>=e&&this._batchRendering();var d=c._node,h=d._texture||(d._spriteFrame?d._spriteFrame._texture:null),k=d._blendFunc.src,d=d._blendFunc.dst,
r=c._shaderProgram;if(b||a.texture!==h||a.blendSrc!==k||a.blendDst!==d||a.shader!==r)this._batchRendering(),a.texture=h,a.blendSrc=k,a.blendDst=d,a.shader=r,b=!1;h=c.uploadData(m,n,6*f);if(0<h){switch(c.vertexType||u.QUAD){case u.QUAD:for(c=0;c<h;c+=4)k=f+c,p[g++]=k+0,p[g++]=k+1,p[g++]=k+2,p[g++]=k+1,p[g++]=k+2,p[g++]=k+3;break;case u.TRIANGLE:s=!1;for(c=0;c<h;c+=3)k=f+c,p[g++]=k+0,p[g++]=k+1,p[g++]=k+2;break;case u.CUSTOM:s=!1;c.uploadIndexData&&(g+=c.uploadIndexData(p,g,f));break;default:return}f+=
...
a[2]=n;a[3]=s;a[4]=b;a[6]=p;a[7]=u;a[8]=c;a[9]=k;a[11]=t;a[12]=d;a[13]=m;a[14]=r;return this};a.kmMat4Multiply=function(a,b,c){var d=a.mat,k=b.mat,m=c.mat;c=k[0];b=k[1];var n=k[2],p=k[3],r=k[4],s=k[5],u=k[6],t=k[7],x=k[8],y=k[9],v=k[10],A=k[11],w=k[12],z=k[13],C=k[14],k=k[15],B=m[0],D=m[1],E=m[2],F=m[3],G=m[4],H=m[5],I=m[6],J=m[7],K=m[8],L=m[9],M=m[10],N=m[11],O=m[12],P=m[13],Q=m[14],m=m[15];d[0]=B*c+D*r+E*x+F*w;d[1]=B*b+D*s+E*y+F*z;d[2]=B*n+D*u+E*v+F*C;d[3]=B*p+D*t+E*A+F*k;d[4]=G*c+H*r+I*x+J*w;d[5]=
G*b+H*s+I*y+J*z;d[6]=G*n+H*u+I*v+J*C;d[7]=G*p+H*t+I*A+J*k;d[8]=K*c+L*r+M*x+N*w;d[9]=K*b+L*s+M*y+N*z;d[10]=K*n+L*u+M*v+N*C;d[11]=K*p+L*t+M*A+N*k;d[12]=O*c+P*r+Q*x+m*w;d[13]=O*b+P*s+Q*y+m*z;d[14]=O*n+P*u+Q*v+m*C;d[15]=O*p+P*t+Q*A+m*k;return a};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var d=b[1],k=b[2],m=b[3],n=b[4],p=b[5],r=b[6],s=b[7],u=b[8],t=b[9],x=b[10],y=b[11],v=b[12],A=b[13],w=b[14],z=b[15],C=c[0],B=c[1],D=c[2],E=c[3],F=c[4],G=c[5],H=c[6],I=c[7],J=c[8],K=c[9],L=c[10],M=c[11],N=c[12],
O=c[13],P=c[14],c=c[15];b[0]=C*a+B*n+D*u+E*v;b[1]=C*d+B*p+D*t+E*A;b[2]=C*k+B*r+D*x+E*w;b[3]=C*m+B*s+D*y+E*z;b[4]=F*a+G*n+H*u+I*v;b[5]=F*d+G*p+H*t+I*A;b[6]=F*k+G*r+H*x+I*w;b[7]=F*m+G*s+H*y+I*z;b[8]=J*a+K*n+L*u+M*v;b[9]=J*d+K*p+L*t+M*A;b[10]=J*k+K*r+L*x+M*w;b[11]=J*m+K*s+L*y+M*z;b[12]=N*a+O*n+P*u+c*v;b[13]=N*d+O*p+P*t+c*A;b[14]=N*k+O*r+P*x+c*w;b[15]=N*m+O*s+P*y+c*z;return this};a.getMat4MultiplyValue=function(a,b){var c=a.mat,d=b.mat,k=new Float32Array(16);k[0]=c[0]*d[0]+c[4]*d[1]+c[8]*d[2]+c[12]*d[3];
...
x,y,v,A,w,z,C=a.length,g=m.length=0;g<C;g+=2)k=a[(g-2+C)%C],x=a[(g-1+C)%C],y=a[g],v=a[g+1],A=a[(g+2)%C],w=a[(g+3)%C],f.x=x-v,f.y=y-k,h.x=v-w,h.y=A-y,cc.pNormalizeIn(f),cc.pNormalizeIn(h),v=f.x*h.x+f.y*h.y+1,z=(f.x+h.x)/v,v=(f.y+h.y)/v,m.push(z,v,h.x,h.y);C/=2;z=0<d;if(this._ensureCapacity(this._vertexCount+3*(3*C-2))){for(var B=!1==z?0.5:0,g=0;g<C-2;g++)k=a[0]-m[0]*B,x=a[1]-m[1]*B,y=a[2*g+2]-m[4*(g+1)]*B,v=a[2*g+3]-m[4*(g+1)+1]*B,A=a[2*g+4]-m[4*(g+2)]*B,w=a[2*g+5]-m[4*(g+2)+1]*B,this.appendVertexData(k,
x,c,0,0),this.appendVertexData(y,v,c,0,0),this.appendVertexData(A,w,c,0,0);var D,E,F,G;d=z?d:0.5;c=z?e:c;for(g=0;g<C;g++)e=(g+1)%C,k=a[2*g],x=a[2*g+1],y=a[2*e],v=a[2*e+1],f.x=m[4*g+2],f.y=m[4*g+3],h.x=z?-f.x:0,h.y=z?-f.y:0,D=m[4*g],E=m[4*g+1],F=m[4*e],G=m[4*e+1],e=k-D*d,A=x-E*d,w=y-F*d,B=v-G*d,k+=D*d,x+=E*d,y+=F*d,v+=G*d,this.appendVertexData(e,A,c,h.x,h.y),this.appendVertexData(w,B,c,h.x,h.y),this.appendVertexData(y,v,c,f.x,f.y),this.appendVertexData(e,A,c,h.x,h.y),this.appendVertexData(k,x,c,f.x,
f.y),this.appendVertexData(y,v,c,f.x,f.y);m.length=0;b.length=0;this._dirty=!0}}},_drawSegments:function(a,b,c,d){b=null==b?this._lineWidth:b;if(!(0>=b)){c=c||this._drawColor;null==c.a&&(c.a=255);b*=0.5;var e,g,k,y,v,A,w,z=a.length;for(w=m.length=0;w<z;w+=2)e=a[(w-2+z)%z],g=a[(w-1+z)%z],k=a[w],y=a[w+1],v=a[(w+2)%z],A=a[(w+3)%z],f.x=g-y,f.y=k-e,h.x=y-A,h.y=v-k,cc.pNormalizeIn(f),cc.pNormalizeIn(h),k=f.x*h.x+f.y*h.y+1,y=(f.x+h.x)/k,k=(f.y+h.y)/k,m.push(y,k,h.x,h.y);z/=2;if(this._ensureCapacity(this._vertexCount+
3*(3*z-2))){d=d?z:z-1;var C,B,D,E,F,G;for(w=0;w<d;w++)v=(w+1)%z,e=a[2*w],g=a[2*w+1],k=a[2*v],y=a[2*v+1],f.x=m[4*w+2],f.y=m[4*w+3],C=m[4*w],B=m[4*w+1],D=m[4*v],E=m[4*v+1],v=e-C*b,A=g-B*b,F=k-D*b,G=y-E*b,e+=C*b,g+=B*b,k+=D*b,y+=E*b,this.appendVertexData(v,A,c,-f.x,-f.y),this.appendVertexData(F,G,c,-f.x,-f.y),this.appendVertexData(k,y,c,f.x,f.y),this.appendVertexData(v,A,c,-f.x,-f.y),this.appendVertexData(e,g,c,f.x,f.y),this.appendVertexData(k,y,c,f.x,f.y);m.length=0;this._dirty=!0}}},clear:function(){this.release();
...
t.rect.y,t.rect.width,t.rect.height),x=cc.rectPixelsToPoints(x);x.x+=this._imageOffset.x;x.y+=this._imageOffset.y;var y=!1;if(this._spriteFrame){var v=this._spriteFrame._rect;if(this._spriteFrame._rotated){var y=!0,A=x.x;x.x=x.y+v.x;x.y=A+v.y}else x.x+=v.x,x.y+=v.y}(v=this.getChildByTag(k))?c._updateCharTexture(v,x,p,y):(v=new cc.Sprite,v.initWithTexture(d,x,y),v._newTextureWhenChangeColor=!0,this.addChild(v,0,k));v.opacityModifyRGB=this._opacityModifyRGB;c._updateCharColorAndOpacity(v);x=cc.p(e+
t.xOffset+0.5*t.rect.width+u,h+(m.commonHeight-t.yOffset)-0.5*x.height*cc.contentScaleFactor());v.setPosition(cc.pointPixelsToPoints(x));e+=t.xAdvance+u;u=p;g<e&&(g=e)}f.width=t&&t.xAdvance<t.rect.width?g-t.xAdvance+t.rect.width:g;f.height=s;this.setContentSize(cc.sizePixelsToPoints(f))},updateString:function(a){var b=this._children;if(b)for(var c=0,d=b.length;c<d;c++){var e=b[c];e&&(e.visible=!1)}this._config&&this._string&&0<this._string.length&&this.createFontChars();a||this.updateLabel()},getString:function(){return this._initialString},
...
t=u%a._itemsPerRow,x=0|u/a._itemsPerRow;if(!(0>t||0>x||t*n+n>g||x*p+p>f)){s++;var y;cc.FIX_ARTIFACTS_BY_STRECHING_TEXEL?(t=(2*t*h+1)/(2*g),u=t+(2*h-2)/(2*g),x=(2*x*k+1)/(2*f),y=x+(2*k-2)/(2*f)):(t=t*h/g,u=t+h/g,x=x*k/f,y=x+k/f);var v=m[r],A=v.tl,w=v.tr,z=v.bl,v=v.br;A.texCoords.u=t;A.texCoords.v=x;w.texCoords.u=u;w.texCoords.v=x;z.texCoords.u=t;z.texCoords.v=y;v.texCoords.u=u;v.texCoords.v=y;z.vertices.x=s*n;z.vertices.y=0;z.vertices.z=0;v.vertices.x=s*n+n;v.vertices.y=0;v.vertices.z=0;A.vertices.x=
...
a._addChild=function(){}})();cc._globalFontSize=cc.ITEM_SIZE;cc._globalFontName="Arial";cc._globalFontNameRelease=!1;
...
c;if(b&&0<b.length)for(var d=b.length-1;0<=d;d--)if(c=b[d],c.isVisible()&&c.isEnabled()){var e=c.convertToNodeSpace(a),f=c.rect();f.x=0;f.y=0;if(cc.rectContainsPoint(f,e))return c}return null}});_p=cc.Menu.prototype;cc.Menu.create=function(a){var b=arguments.length;0<b&&null==arguments[b-1]&&cc.log("parameters should not be ending with null in Javascript");return 0===b?new cc.Menu:1===b?new cc.Menu(a):new cc.Menu(Array.prototype.slice.call(arguments,0))};var g_resources="res/1.png res/2.png res/3.png res/4.png res/5.png res/bg.jpg".split(" ");var Constant={CANDY_WIDTH:64,CANDY_TYPE_COUNT:5,MAP_SIZE:10,FALL_ACCELERATION:40,levels:[{limitStep:30,targetScore:500},{limitStep:25,targetScore:1E3},{limitStep:20,targetScore:2E3},{limitStep:20,targetScore:3E3},{limitStep:15,targetScore:4E3},{limitStep:10,targetScore:5E3}]};var Storage={getCurrentLevel:function(){var a=cc.sys.localStorage.getItem("level")||0;return parseInt(a)},setCurrentLevel:function(a){cc.sys.localStorage.setItem("level",a)},getCurrentScore:function(){var a=cc.sys.localStorage.getItem("score")||0;return parseInt(a)},setCurrentScore:function(a){cc.sys.localStorage.setItem("score",a)}};var GameUI=cc.Layer.extend({levelText:null,scoreText:null,stepText:null,gameLayer:null,ctor:function(a){this._super();this.gameLayer=a;this.initInfoPanel();this.scheduleUpdate()},initInfoPanel:function(){var a=cc.director.getWinSize(),b=new cc.LabelTTF("Level","arial",36);b.x=100;b.y=a.height-40;b.setColor(cc.color(0,0,0));this.addChild(b);a=new cc.LabelTTF("1","arial",36);a.x=100;a.y=b.y-40;a.setColor(cc.color(0,0,0));this.addChild(a);this.levelText=a;var c=new cc.LabelTTF("Score","arial",36);c.x=
370;c.y=b.y;c.setColor(cc.color(0,0,0));this.addChild(c);c=new cc.LabelTTF("1","arial",36);c.x=370;c.y=a.y;c.setColor(cc.color(0,0,0));this.addChild(c);this.scoreText=c;c=new cc.LabelTTF("Step","arial",36);c.x=620;c.y=b.y;c.setColor(cc.color(0,0,0));this.addChild(c);b=new cc.LabelTTF("1","arial",36);b.x=620;b.y=a.y;b.setColor(cc.color(0,0,0));this.addChild(b);this.stepText=b},update:function(){this.levelText.setString(""+this.gameLayer.level-0+1);this.scoreText.setString(""+this.gameLayer.score);
